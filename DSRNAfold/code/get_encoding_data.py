...
def get_encoding_data(file_type, data_path, file_num, repr_dim, name, limit_length, Save_dir, start_id, end_id, whether_pack_train, whether_pack_test):
    ...
    for i in range(len(file_num)):
        ...
        if file_num[i] == name.split('_')[0] + '-{}'.format(limit_length) and whether_pack_train == True: 
            ...
            if file_type == 'bpseq':
                train_seq_list, train_label_matrix, train_length, file_name = get_bpseq_table(path, limit_length, start_id, end_id)
            elif file_type == 'ct':
                train_seq_list, train_label_matrix, train_length, file_name = get_ct_table(path, limit_length, start_id, end_id)
            ...
            print('get test data ...')
            if file_type == 'bpseq':
                test_seq_list, test_label_matrix, test_length, file_name = get_bpseq_table(path, limit_length, start_id, end_id)
            elif file_type == 'ct':
                test_seq_list, test_label_matrix, test_length, file_name = get_ct_table(path, limit_length, start_id, end_id)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Get encoding data')

    parser.add_argument('--pack_start_id', type=int,  help='The start id of pack in LTP_get_data')
    parser.add_argument('--pack_end_id', type=int, help='The end id of pack in LTP_get_data')

    args_from_cmd = parser.parse_args()
    pack_start_id = args_from_cmd.pack_start_id
    pack_end_id = args_from_cmd.pack_end_id
    ...
    get_encoding_data(file_type, data_path, file_num, repr_dim, encoding_model_name, limit_length, Save_dir, 
                      pack_start_id, pack_end_id, whether_pack_train, whether_pack_test)
